...
execute as @a[scores={deaths=1..},tag=!out,tag=!ded,predicate=lobster:spawn] run function lobster:death_gun_game
execute as @a[scores={kills=1..},tag=!out,tag=!ded] run function lobster:kill_gun_game
...
execute unless entity @e[type=item,nbt={Item:{tag:{gungame:1b}}}] run function lobster:gun_game_pickup
