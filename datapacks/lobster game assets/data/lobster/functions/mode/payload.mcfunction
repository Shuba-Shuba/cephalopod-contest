...
execute if score %map game matches 0 in sp:void run tp @a[team=blue] 423.0 90 476.0
execute if score %map game matches 0 in sp:void run tp @a[team=red] 425 90 425
...
# summon payload cart
execute in sp:void run summon minecraft:armor_stand 427 90 472 {Rotation:[-90f,0f],CustomName:'"payload_cart.shb"',active_effects:[{id:"minecraft:glowing",duration:-1,amplifier:0b,show_particles:0b}],Marker:1b,NoBasePlate:1b,Invisible:1b,NoGravity:1b,Tags:["payload"],Passengers:[{id:"minecraft:furnace_minecart",CustomDisplayTile:1b,DisplayState:{Name:"minecraft:redstone_lamp",Properties:{lit:"false"}},DisplayOffset:4,Tags:["payload"],Invulnerable:1b}]}

# reset payload progress counter
scoreboard players set .payload_progress game 0
execute if score %map game matches 0 run bossbar set lobster:time max 3740
scoreboard players set .payload_checkpoint game 0

# setup time
scoreboard players set .payload_setup game 1
execute if score %map game matches 0 in sp:void run fill 431 90 465 431 90 466 air
execute if score %map game matches 0 in sp:void run setblock 424 90 472 air
execute if score %map game matches 0 in sp:void run fill 423 98 477 422 98 477 air