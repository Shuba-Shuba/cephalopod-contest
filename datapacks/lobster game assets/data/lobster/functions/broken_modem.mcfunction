...
execute as @e[type=#lobster:broken_modem_stops,tag=broken_modem_stopped] at @s run function shb:explode_destructive

# stop nearby projectiles
tag @e[type=#lobster:broken_modem_stops,distance=..10] add broken_modem_stopped
execute as @e[type=#lobster:broken_modem_stops,distance=..10] run data merge entity @s {Motion:[0d,0d,0d],NoGravity:1b,Invulnerable:1b}