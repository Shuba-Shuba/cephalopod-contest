...
# choose map
execute if score %map game matches 0 in sp:void run tp @a[tag=!out] -241 118 993 180 0
...
execute store result bossbar lobster:time max run scoreboard players get time.spleef config
execute store result bossbar lobster:time value run scoreboard players get time.spleef config
...
execute in sp:void run function lobster:spleef_start