...
execute if entity @a[tag=winner] run function cc:end
execute unless entity @a[tag=!out,tag=!ded] run function cc:end


# micro battles stuff
gamemode survival @a[tag=!out,tag=!ded,gamemode=adventure]
execute as @a[tag=!out,tag=!ded] at @s if block ~ ~-0.5 ~ barrier run function cc:landed_no_kb
execute if score %second game matches ..9 run function cc:reinforce_walls
execute if score %second game matches 10 if score %tick game matches 0 run function cc:break_walls
scoreboard players add %edge game 1
execute if score %second game matches 10.. if score %edge game matches 90.. run function cc:edge_push
