...
# make armor stand
summon armor_stand ~ ~ ~ {Tags:["init","board_player"],NoBasePlate:1b,NoGravity:1b,Invulnerable:1b,CustomNameVisible:1b,ShowArms:1b}
data modify entity @e[type=armor_stand,limit=1,sort=nearest,tag=init] Rotation set from entity @s Rotation
execute as @a[tag=!out] if score @s board_turn = @e[type=marker,tag=this,limit=1] board_turn run scoreboard players operation @e[type=armor_stand,limit=1,sort=nearest,tag=init] board_turn = @s board_turn
