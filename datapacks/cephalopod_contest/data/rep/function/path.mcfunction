...
execute as @e[type=marker,tag=anchor] if score @s anchor_index = .search anchor_index if score @s anchor_id = .search anchor_id run tp @e[type=mannequin,tag=this,limit=1] @s
...
scoreboard players add @e[type=mannequin,tag=this,limit=1] anchor_index 1
scoreboard players operation .search anchor_id = @s anchor_id
