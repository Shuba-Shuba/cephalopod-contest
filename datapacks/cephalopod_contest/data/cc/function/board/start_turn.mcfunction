...
# reveal player
tag @s add this
execute as @e[type=mannequin,tag=board_player] if score @s board_turn = .i board_turn run tp 0 -1000 0
execute as @e[type=item_display,tag=board_player] if score @s board_turn = .i board_turn at @s run tp @p[tag=this] @s
execute as @e[type=item_display,tag=board_player] if score @s board_turn = .i board_turn at @s run playsound entity.evoker.prepare_wololo master @a ~ ~ ~ 2 1 1
gamemode adventure
