...
execute as @a[scores={deaths=1..},tag=!out,tag=!ded] run function cc:death_payload1
execute as @a[tag=tmp_dead,predicate=cc:spawn] run function cc:death_payload
#execute as @a[scores={kills=1..},tag=!out,tag=!ded] run function cc:kill_payload
scoreboard players add %tick game 1
execute if score %tick game matches 20.. run scoreboard players remove %second game 1
execute if score %tick game matches 20.. as @a[scores={death_time=0}] run function cc:respawn_payload
execute if score %tick game matches 20.. as @a[scores={death_time=1..}] run function cc:death_time_genwars
execute if score %tick game matches 20.. run function cc:clock_payload
execute if score %second game matches ..0 if score .payload_setup game matches 0 run function cc:payload_time_end
execute if score %second game matches ..0 if score .payload_setup game matches 1 run function cc:payload_start
