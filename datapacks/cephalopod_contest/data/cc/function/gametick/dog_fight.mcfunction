...
execute if score %players game matches ..1 run function cc:dog_fight_end
execute if score %players game matches 2.. if score %second game matches 10.. as @e[type=wolf,limit=1,sort=random] at @s run function cc:landed_no_kb
execute as @e[type=wolf] at @s run data modify entity @s angry_at set from entity @e[type=wolf,limit=1,sort=nearest,distance=0.01..] UUID