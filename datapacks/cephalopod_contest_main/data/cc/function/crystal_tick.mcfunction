...
#execute if score @s crystal_count < #previous_tick crystal_count run function cc:arrow_reload
#execute if score @s crystal_count matches 0 if score #previous_tick crystal_count matches 2 run function cc:arrow_reload
...
execute if score @s crystal_count matches 0 if score #tmp arrow_count matches 0 run function cc:arrow_reload
execute if score @s crystal_count matches 0 if score #tmp arrow_count matches 0..1 run function cc:arrow_reload
execute if score @s crystal_count matches 1 if score #tmp arrow_count matches 0 run function cc:arrow_reload

## aim crystal beam at tagged block or entity
# find tagged block or entity
execute as @e[type=#cc:triangulator_tags,tag=tnt_arc,tag=!out,tag=!ded] if score @s tagUUID0 = .search UUID0 if score @s tagUUID1 = .search UUID1 if score @s tagUUID2 = .search UUID2 if score @s tagUUID3 = .search UUID3 run tag @s add arc_tagged
...
execute unless data entity @s Inventory[{Slot:1b,id:"minecraft:bow"}] unless entity @s[tag=suppress_cd] unless score @s triangulator_cd matches 1.. unless entity @e[type=#cc:triangulator_tags,tag=arc_tagged] run scoreboard players set @s triangulator_cd 70
...
# warp
execute if entity @s[tag=arc_warp] if entity @e[type=#cc:triangulator_tags,tag=arc_tagged] run function cc:arc_warp

# reatomize
execute if entity @s[tag=arc_reatomize] run function cc:arc_reatomize

# reset
tag @s remove arc_reatomize
tag @s remove arc_warp
tag @s remove suppress_cd
tag @e[type=#cc:triangulator_tags] remove arc_tagged
