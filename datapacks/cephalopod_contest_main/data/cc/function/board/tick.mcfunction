...
execute if score .anim board_roll matches 1.. as @e[type=item_display,tag=board_player] if score @s board_turn = .i board_turn at @s run tp ^ ^ ^0.2
execute if score .anim board_roll matches 1.. as @e[type=item_display,tag=board_player] if score @s board_turn = .i board_turn in cc:void positioned as @s as @a[tag=!out] if score @s board_turn = .i board_turn run tp @s ~ ~ ~
execute if score .anim board_roll matches 1.. as @e[type=item_display,tag=board_player] if score @s board_turn = .i board_turn in cc:void positioned as @s as @e[type=zombie,tag=board_player_placeholder] if score @s board_turn = .i board_turn run tp @s ~ ~ ~
execute if score .anim board_roll matches 0.. run scoreboard players remove .anim board_roll 1
execute if score .anim board_roll matches 0 as @e[type=item_display,tag=board_player] if score @s board_turn = .i board_turn at @s run function cc:board/move_check
